...
- 分享短链：每个主要内容页还有一个 `/s/xxxx` 短链，由 `scripts/gen_shortlinks.py` 按文件路径哈希生成，写在 front matter 的 `shortlink` 字段和 `aliases` 里。启用仓库 hook（`bash scripts/install-hooks.sh`）后会在 commit 时自动生成，也可手工运行脚本补齐；**不要手工修改已发布页面的 `shortlink`**，那会让已经分享出去的短链失效。
